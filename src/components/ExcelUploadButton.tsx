...
export function ExcelUploadButton({ 
  ...
  useEffect(() => {
    ...
    // 초기 상태는 파일 없음 (쿠키에 파일명이 있을 때만 표시)
    if (savedFileName) {
  ...
  const buttonClass = className || (isNavigation 
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      {/* 현재 로드된 파일명 표시 - 버튼 아래로 이동 */}
      {loadedFileName && !isNavigation && (
        <div className="flex items-center gap-1 px-3 py-1.5 bg-gray-100 rounded-md text-sm text-gray-600">
          <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
          <span className="max-w-[200px] truncate" title={loadedFileName}>
            {loadedFileName}
          </span>
        </div>
      )}
